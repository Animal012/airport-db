...
void onb2::selectAll()
{
    ...
    QString sqlstr = "SELECT * FROM Check_in_counter ORDER BY counter_id";
    ...
    while (query.next())
    {
        ...
        QTableWidgetItem *idItem = new QTableWidgetItem(query.value("counter_id").toString());
        ui->tw->setItem(rowCount, 0, idItem);

        // Заполняем значениями из базы данных
        ui->tw->setItem(rowCount, 1, new QTableWidgetItem(query.value("counter_number").toString()));
...
void onb2::add()
{
    ...
    QString sqlstr = "insert into Check_in_counter(counter_number) values(?)";  // Формируем строку SQL-запроса для вставки данных в таблицу
...
void onb2::remove()
{
    ...
    QString sqlstr = "DELETE FROM Check_in_counter WHERE counter_id = '" + pu1 + "'";
...
void onb2::edit()
{
    ...
    QString sqlstr = "UPDATE Check_in_counter SET counter_number = ? WHERE counter_id = ?";
...
void onb2::generateReportRequest() {
    ...
    QString reportText = "Запрос на формирование отчета - о Стойках:\n\n";
    reportText += "Уважаемые коллеги,\n\n";
    reportText += "Подготовлен отчет о Стойках:\n";
    ...
    for (int row = 0; row < ui->tw->rowCount(); ++row) {
        ...
        reportText +=  " ID: " + pu1 + " ID: " + pu2 + ";\n";
    }

    reportText += "\nС уважением,\n";
    reportText += "Ваше имя";

    QString fileName = "Стойки_report_request.txt"; // Имя файла для сохранения отчета
