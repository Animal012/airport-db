    ...
    , ui(new Ui::onb5)
{
    ...
    ui->tw->setHorizontalHeaderItem(1, new QTableWidgetItem("Модель"));
    ui->tw->setHorizontalHeaderItem(2, new QTableWidgetItem("Компания"));
    ui->tw->setHorizontalHeaderItem(3, new QTableWidgetItem("Ввод в эксплатацию"));
...
void onb5::selectAll()
{
    ...
    QString sqlstr = "SELECT * FROM Aircraft ORDER BY aircraft_id";
    ...
    while (query.next())
    {
        ...
        QTableWidgetItem *idItem = new QTableWidgetItem(query.value("aircraft_id").toString());
        ui->tw->setItem(rowCount, 0, idItem);

        // Заполняем значениями из базы данных
        ui->tw->setItem(rowCount, 1, new QTableWidgetItem(query.value("model").toString()));
        ui->tw->setItem(rowCount, 2, new QTableWidgetItem(query.value("airline").toString()));
        ui->tw->setItem(rowCount, 3, new QTableWidgetItem(query.value("entry_into_service").toString()));
...
void onb5::add()
{
    ...
    QString sqlstr = "insert into Aircraft(model, airline, entry_into_service) values(?, ?, ?)";  // Формируем строку SQL-запроса для вставки данных в таблицу
...
void onb5::remove()
{
    ...
    QString sqlstr = "DELETE FROM Aircraft WHERE aircraft_id = '" + pu1 + "'";
...
void onb5::edit()
{
    ...
    QString sqlstr = "UPDATE Aircraft SET model = ?, airline = ?, entry_into_service = ? WHERE aircraft_id = ?";
...
void onb5::generateReportRequest() {
    ...
    QString reportText = "Запрос на формирование отчета - о Самолетах:\n\n";
    reportText += "Уважаемые коллеги,\n\n";
    reportText += "Подготовлен отчет о Самолетах:\n";
    ...
    for (int row = 0; row < ui->tw->rowCount(); ++row) {
        ...
        reportText +=  " Модель: " + pu1 + " Компания: " + pu2 + " Ввод в экплатацию: " + pu3 + ";\n";
    }

    reportText += "\nС уважением,\n";
    reportText += "Ваше имя";

    QString fileName = "Самолеты_report_request.txt"; // Имя файла для сохранения отчета
