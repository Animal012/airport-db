    ...
    , ui(new Ui::onb6)
{
    ...
    ui->tw->setHorizontalHeaderItem(1, new QTableWidgetItem("Код ИАТА"));
    ui->tw->setHorizontalHeaderItem(2, new QTableWidgetItem("Город"));
    ui->tw->setHorizontalHeaderItem(3, new QTableWidgetItem("Страна"));
...
void onb6::selectAll()
{
    ...
    QString sqlstr = "SELECT * FROM Destination ORDER BY destination_id";
    ...
    while (query.next())
    {
        ...
        QTableWidgetItem *idItem = new QTableWidgetItem(query.value("destination_id").toString());
        ui->tw->setItem(rowCount, 0, idItem);

        // Заполняем значениями из базы данных
        ui->tw->setItem(rowCount, 1, new QTableWidgetItem(query.value("IATA_code").toString()));
        ui->tw->setItem(rowCount, 2, new QTableWidgetItem(query.value("city").toString()));
        ui->tw->setItem(rowCount, 3, new QTableWidgetItem(query.value("country").toString()));
...
void onb6::add()
{
    ...
    // Проверяем, что поля city и country содержат только буквы
    QRegularExpression rx("^[A-Za-z ]+$");
    if (!rx.match(pu2).hasMatch() || !rx.match(pu3).hasMatch())
    {
        QMessageBox::critical(this, "Ошибка", "Поля city и country должны содержать только буквы.");
        return;
    }

    QSqlQuery query;  // Создаем объект запроса

    QString sqlstr = "insert into Destination(IATA_code, city, country) values(?, ?, ?)";  // Формируем строку SQL-запроса для вставки данных в таблицу
...
void onb6::remove()
{
    ...
    QString sqlstr = "DELETE FROM Destination WHERE destination_id = '" + pu1 + "'";
...
void onb6::edit()
{
    ...
    QString pu1 = ui->le1->text(); // destination_id
    QString pu2 = ui->le2->text(); // IATA_code
    QString pu3 = ui->le3->text(); // city
    QString pu4 = ui->le4->text(); // country

    // Проверяем, что поля city и country содержат только буквы
    QRegularExpression rx("^[A-Za-z ]+$");
    if (!rx.match(pu3).hasMatch() || !rx.match(pu4).hasMatch())
    {
        QMessageBox::critical(this, "Ошибка", "Поля city и country должны содержать только буквы.");
        return;
    }
    ...
    QString sqlstr = "UPDATE Destination SET IATA_code = ?, city = ?, country = ? WHERE destination_id = ?";
...
void onb6::generateReportRequest() {
    ...
    QString reportText = "Запрос на формирование отчета - о Пункстах назначения:\n\n";
    reportText += "Уважаемые коллеги,\n\n";
    reportText += "Подготовлен отчет о Пункстах назначения:\n";
    ...
    for (int row = 0; row < ui->tw->rowCount(); ++row) {
        ...
        reportText +=  " Код ИАТА: " + pu1 + " Город: " + pu2 + " Страна: " + pu3 + ";\n";
    }

    reportText += "\nС уважением,\n";
    reportText += "Ваше имя";

    QString fileName = "Пунктназначения_report_request.txt"; // Имя файла для сохранения отчета
